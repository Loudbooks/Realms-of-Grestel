    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        if (args.isNotEmpty()){
            if (args[0] == "kick"){
                ...
                if (claimManager.getClaimByPlayer(grestelPlayer) != null && claimManager.getClaimByPlayer(grestelPlayer)!!.owner == grestelPlayer){
                    ...
                    return if (claim.players.contains(playerManager.getPlayer(player))){
                        ...
                        for (player1 in claim.players) {
                            if (player1 != grestelPlayer) {
                                player1.player.sendMessage("${ChatColor.GREEN}${player.name} has been kicked from ${claim.name}!")
                            }
                        }
                        true
                    } else {
                ...
                if (claimManager.getClaimByPlayer(grestelPlayer) != null && claimManager.getClaimByPlayer(grestelPlayer)!!.owner == grestelPlayer){
                    ...
                    return if (args[1].length < 15){
                        ...
                        for (player in claim.players) {
                            if (player != grestelPlayer) {
                                player.player.sendMessage("${ChatColor.GREEN}${sender.name} has renamed the claim to \"${claim.name}\"!")
                            }
                        }
                        true
                    } else {
                ...
                if (claimManager.getClaimByPlayer(grestelPlayer) != null && claimManager.getClaimByPlayer(grestelPlayer)!!.owner == grestelPlayer) {
                    ...
                    if (claim.players.contains(playerManager.getPlayer(player))){
                ...
                if (claimManager.getClaimByPlayer(grestelPlayer) != null){
                    ...
                    for (player in claim.players) {
                        if (player != grestelPlayer) {
                            player.player.sendMessage("${ChatColor.GREEN}${sender.name} has left ${claim.name}!")
                        }
                    }
                    return true
                }
                ...
                if (claimManager.getClaimByPlayer(grestelPlayer) != null && claimManager.getClaimByPlayer(grestelPlayer)!!.owner == grestelPlayer){
                    ...
                    for (player in claim.players) {
                        if (player != grestelPlayer) {
                            player.player.sendMessage("${ChatColor.GREEN}${sender.name} has disbanded ${claim.name}!")
                        }
                    }
                    return true
                }
            ...
            if (claimManager.getClaimByPlayer(grestelPlayer) != null) {
                ...
                val claim = Claim(grestelPlayer, 5, name)
                claim.chunks.add(Pair(sender.location.block.x.toDouble(), sender.location.block.z.toDouble()))
                claim.numberOfChunksAvailable--
